...
This encoding is used by the LBRNode RPC API to transport binary data in JSON payloads.
...
var (
	...
	ErrLBRVersion   = &decError{"LBR version error"}
)
...
func Decode(input string) ([]byte, error) {
	...
	if !has0xPrefix(input) {
		...
		if input[0] == 'm' {
			...
			res, outVersion, err := base58util.LBRDecode(input)

			if err != nil {
				log.Debugf("Decode address as LBR address failed")
				return nil, err
			} else if outVersion != LBRVersion {
				//fmt.Printf("LBR version mismatch: %X .vs. %X\n", outVersion, LBRVersion)
				return nil, ErrLBRVersion
			}
			return res, err
